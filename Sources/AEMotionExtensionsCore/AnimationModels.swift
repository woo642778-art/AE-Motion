...
public enum KeyframeValue: Equatable, Codable, Sendable {
    ...
    public func interpolated(to other: KeyframeValue, progress: Double, clamping: Bool = true) -> KeyframeValue {
        ...
        case let (.angle(a), .angle(b)):
            return .angle(a + shortestAngleDelta(from: a, to: b) * t)
        case let (.point(ax, ay), .point(bx, by)):
            return .point(x: ax + (bx - ax) * t, y: ay + (by - ay) * t)
    ...
    public func adding(_ other: KeyframeValue) -> KeyframeValue? {
        ...
        case let (.color(ar, ag, ab, aa), .color(br, bg, bb, ba)):
            return .color(r: ar + br, g: ag + bg, b: ab + bb, a: aa + ba)
        default: return nil
        }
    }
...
public struct BezierHandle: Equatable, Codable, Sendable {
    ...
    public init(timeOffset: Double = 0.3333333333, valueOffset: Double = 0) {
        self.timeOffset = timeOffset
        self.valueOffset = valueOffset
    }
}

public enum InterpolationMode: String, CaseIterable, Codable, Sendable {
    case hold
    case linear
    case autoBezier
    case bezier
...
public enum ExtrapolationMode: String, CaseIterable, Codable, Sendable {
    case clamp
    case linear
    case cycle
    case pingPong
...
public struct Keyframe: Identifiable, Equatable, Codable, Sendable {
    ...
    public init(
        ...
        time: Double,
        value: KeyframeValue,
        interpolation: InterpolationMode = .autoBezier,
        inHandle: BezierHandle = .init(timeOffset: -0.3333333333),
        outHandle: BezierHandle = .init(timeOffset: 0.3333333333)
    ) {
        self.id = id
        self.time = time
        self.value = value
...
public struct KeyframeCurve: Equatable, Codable, Sendable {
    ...
    public init(
        ...
        self.keyframes = Self.normalized(keyframes)
        self.preExtrapolation = preExtrapolation
        self.postExtrapolation = postExtrapolation
    }

    public mutating func replaceKeyframes(_ keyframes: [Keyframe]) {
        self.keyframes = Self.normalized(keyframes)
    }

    public mutating func upsert(_ keyframe: Keyframe, tolerance: Double = 1e-9) {
        if let index = keyframes.firstIndex(where: { abs($0.time - keyframe.time) <= tolerance }) {
    ...
    public mutating func remove(ids: Set<UUID>) {
        keyframes.removeAll { ids.contains($0.id) }
    }
    ...
    public func value(at rawTime: Double, fallback: KeyframeValue) -> KeyframeValue {
        ...
        if time <= first.time {
            return extrapolatedValue(at: time, boundary: .pre, fallback: fallback)
        }
        if time >= last.time {
            return extrapolatedValue(at: time, boundary: .post, fallback: fallback)
        }

        guard let upperIndex = keyframes.firstIndex(where: { $0.time >= time }), upperIndex > 0 else {
            return last.value
        }
        let lower = keyframes[upperIndex - 1]
        let upper = keyframes[upperIndex]
        let duration = max(upper.time - lower.time, 1e-12)
        let local = min(max((time - lower.time) / duration, 0), 1)
        let eased = easingProgress(local, mode: lower.interpolation, lower: lower, upper: upper)
        return lower.value.interpolated(to: upper.value, progress: eased)
    }

    public func speed(at time: Double, fallback: KeyframeValue, delta: Double = 1.0 / 600.0) -> Double {
        guard delta > 0 else { return 0 }
        let before = value(at: time - delta, fallback: fallback)
        let after = value(at: time + delta, fallback: fallback)
        let distance = before.distance(to: after)
    ...
    public func valueRange(fallback: KeyframeValue) -> ClosedRange<Double> {
        ...
        guard let minimum = values.min(), let maximum = values.max() else {
            let value = fallback.components.first ?? 0
            return value...value
        }
    ...
    private func extrapolatedValue(at time: Double, boundary: Boundary, fallback: KeyframeValue) -> KeyframeValue {
        ...
        switch mode {
        case .clamp, .cycle, .pingPong:
            return boundary == .pre ? first.value : last.value
        case .linear:
            ...
            if boundary == .pre {
                ...
                a = keyframes[keyframes.count - 2]
                b = keyframes[keyframes.count - 1]
            }
            let duration = max(b.time - a.time, 1e-12)
            let progress = (time - a.time) / duration
            return a.value.interpolated(to: b.value, progress: progress, clamping: false)
        }
    }

    private func remappedTime(_ time: Double, first: Double, last: Double) -> Double {
        guard last > first else { return first }
        if time >= first && time <= last { return time }
        let mode = time < first ? preExtrapolation : postExtrapolation
        let duration = last - first
        switch mode {
        case .clamp, .linear:
            return time
        case .cycle:
            var offset = (time - first).truncatingRemainder(dividingBy: duration)
            if offset < 0 { offset += duration }
            return first + offset
        case .pingPong:
            let period = duration * 2
            var offset = (time - first).truncatingRemainder(dividingBy: period)
    ...
    private func easingProgress(_ t: Double, mode: InterpolationMode, lower: Keyframe, upper: Keyframe) -> Double {
        ...
        case .autoBezier:
            return t * t * (3 - 2 * t)
        case .bezier:
            let c1 = min(max(0.3333333333 + lower.outHandle.valueOffset, -2), 3)
            let c2 = min(max(0.6666666667 + upper.inHandle.valueOffset, -2), 3)
    ...
    private static func normalized(_ keyframes: [Keyframe]) -> [Keyframe] {
        ...
        for keyframe in sorted where keyframe.time.isFinite {
            if let last = result.last, abs(last.time - keyframe.time) <= 1e-9 {
...
public struct AnimatableProperty: Identifiable, Equatable, Codable, Sendable {
    ...
    public func value(at time: Double) -> KeyframeValue {
        guard isEnabled else { return defaultValue }
        return curve.value(at: time, fallback: defaultValue)
    }
}
...
public struct TimeMapping: Equatable, Codable, Sendable {
    ...
    public init(sourceDuration: Double, outputDuration: Double, curve: KeyframeCurve) {
        self.sourceDuration = max(sourceDuration, 0)
        self.outputDuration = max(outputDuration, 0)
        self.curve = curve
    }

    public func sourceTime(at outputTime: Double) -> Double {
        let fallback = KeyframeValue.scalar(min(max(outputTime, 0), sourceDuration))
        guard case let .scalar(value) = curve.value(at: outputTime, fallback: fallback) else {
            return min(max(outputTime, 0), sourceDuration)
        }
...
public struct PropertyBinding: Identifiable, Equatable, Codable, Sendable {
    ...
    public init(
        ...
        target: PropertyID,
        scale: Double = 1,
        offset: KeyframeValue? = nil,
        clamp: ClosedRange<Double>? = nil
    ) {
        self.id = id
        self.source = source
        self.target = target
    ...
    public func transform(_ value: KeyframeValue) -> KeyframeValue {
        var result = value.scaled(by: scale)
        if let offset, let added = result.adding(offset) { result = added }
        if let clamp, case let .scalar(number) = result {
            result = .scalar(min(max(number, clamp.lowerBound), clamp.upperBound))
        }
        return result
    }
}
