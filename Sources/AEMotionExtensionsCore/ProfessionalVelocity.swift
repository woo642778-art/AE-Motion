...
public enum FrameInterpolationMode: String, CaseIterable, Codable, Sendable {
    case nearest
    case frameBlend
    case opticalFlow
}

public enum VelocityAudioMode: String, CaseIterable, Codable, Sendable {
    case linked
    case detached
    case pitchPreserved
    case muted
...
public struct VelocityKeyframe: Identifiable, Equatable, Codable, Sendable {
    ...
    public init(id: UUID = UUID(), time: Double, speed: Double, interpolation: InterpolationMode = .linear) {
        self.id = id
        self.time = time
        self.speed = speed
...
public struct VelocityCurve: Equatable, Codable, Sendable {
    ...
    public init(keyframes: [VelocityKeyframe], sourceOrigin: Double = 0) {
    ...
    public static func constant(duration: Double, speed: Double, sourceOrigin: Double = 0) -> VelocityCurve {
        VelocityCurve(keyframes: [
            VelocityKeyframe(time: 0, speed: speed, interpolation: .linear),
            VelocityKeyframe(time: max(duration, 0), speed: speed, interpolation: .linear),
        ], sourceOrigin: sourceOrigin)
    }

    public mutating func replaceKeyframes(_ keyframes: [VelocityKeyframe]) {
        self.keyframes = Self.normalized(keyframes)
    }
    ...
    public func speed(at time: Double) -> Double {
        ...
        if time <= first.time { return first.speed }
        ...
        case .back:
            let x = t - 1
            eased = 1 + 2.70158 * x * x * x + 1.70158 * x * x
        case .bounce:
            eased = min(max(t + sin(t * .pi * 4) * (1 - t) * 0.15, 0), 1)
        case .elastic:
            eased = t == 0 || t == 1 ? t : pow(2, -10 * t) * sin((t * 10 - 0.75) * (2 * .pi / 3)) + 1
        case .hold: eased = 0
        case .linear: eased = t
        }
    ...
    public func sourceTime(at outputTime: Double, integrationStep: Double = 1.0 / 240.0) -> Double {
        ...
        let end = outputTime
        let step = max(min(integrationStep, end), 1.0 / 2000.0)
        var time = 0.0
        var accumulated = sourceOrigin
        while time < end {
            let next = min(time + step, end)
            let midpoint = (time + next) * 0.5
            accumulated += speed(at: midpoint) * (next - time)
            time = next
        }
        return accumulated
    }
    ...
    public func timeMapping(outputDuration: Double, sourceDuration: Double, sampleRate: Double = 60) -> TimeMapping {
        let interval = 1 / max(sampleRate, 1)
        var frames: [Keyframe] = []
        var time = 0.0
        while time < outputDuration {
            frames.append(Keyframe(time: time, value: .scalar(min(max(sourceTime(at: time), 0), sourceDuration)), interpolation: .linear))
            time += interval
        }
        ...
        return TimeMapping(sourceDuration: sourceDuration, outputDuration: outputDuration, curve: KeyframeCurve(keyframes: frames))
    }

    private static func normalized(_ keyframes: [VelocityKeyframe]) -> [VelocityKeyframe] {
        let valid = keyframes.filter { $0.time.isFinite && $0.speed.isFinite }.sorted { $0.time < $1.time }
        var result: [VelocityKeyframe] = []
        for keyframe in valid {
...
public struct VelocityMarker: Identifiable, Equatable, Codable, Sendable {
    ...
    public init(id: UUID = UUID(), time: Double, kind: Kind, label: String) {
        self.id = id
        self.time = time
        self.kind = kind
        self.label = label
...
public struct ProfessionalVelocityPlan: Equatable, Codable, Sendable {
    ...
    public init(
        curve: VelocityCurve,
        frameInterpolation: FrameInterpolationMode = .frameBlend,
        audioMode: VelocityAudioMode = .pitchPreserved,
        bpm: Double? = nil,
        flowResetTimes: [Double] = []
    ) {
        self.curve = curve
        self.frameInterpolation = frameInterpolation
        self.audioMode = audioMode
        self.bpm = bpm
        self.flowResetTimes = flowResetTimes.filter(\.isFinite).sorted()
    }

    public static func ramp(duration: Double, from: Double, to: Double) -> ProfessionalVelocityPlan {
        ProfessionalVelocityPlan(curve: VelocityCurve(keyframes: [
            VelocityKeyframe(time: 0, speed: from, interpolation: .autoBezier),
            VelocityKeyframe(time: max(duration, 0), speed: to, interpolation: .linear),
        ]))
    }

    public static func freeze(duration: Double, sourceTime: Double) -> ProfessionalVelocityPlan {
        ProfessionalVelocityPlan(
            curve: .constant(duration: duration, speed: 0, sourceOrigin: sourceTime),
    ...
    public static func reverse(duration: Double, sourceEnd: Double) -> ProfessionalVelocityPlan {
        ProfessionalVelocityPlan(
            curve: .constant(duration: duration, speed: -1, sourceOrigin: sourceEnd),
    ...
    public func markers(outputDuration: Double) -> [VelocityMarker] {
        var result = curve.keyframes.dropFirst().dropLast().map {
            VelocityMarker(time: $0.time, kind: abs($0.speed) < 1e-9 ? .freeze : $0.speed < 0 ? .reverse : .speedChange, label: String(format: "%.2fx", $0.speed))
        }
        if let bpm, bpm > 0 {
            let interval = 60 / bpm
            var time = 0.0
            var beat = 1
            while time <= outputDuration + 1e-9 {
                result.append(VelocityMarker(time: time, kind: .beat, label: "Beat \(beat)"))
                beat += 1
                time += interval
            }
        }
